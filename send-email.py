...
recipients_en = emails_df[emails_df['language'] == 'EN']
...
def prepare_and_send_emails(subject, body_template, sender, recipients, links, password):
    for i, recipient in recipients.iterrows():
        ...
        send_email(subject, personalized_body, sender, recipient, password)

# Send Spanish emails
prepare_and_send_emails(subject_es, body_es, sender, recipients_es, links_es, password)

# Send English emails
prepare_and_send_emails(subject_en, body_en, sender, recipients_en, links_en, password)
